...
def riemann_flux(Axis, Left_state: ti.math.vec4, Right_state: ti.math.vec4, gamma):
    ...
    # fluxes (local Lax-Friedrichs/Rusanov)
    if Axis == 0: # flux for radial terms
        flux_rho   = rho_star * u_star
        flux_mom_u = rho_star * u_star**2
        flux_mom_w = rho_star * u_star * w_star
        ...
        flux_mom_u = rho_star * u_star * w_star 
        flux_mom_w = rho_star * w_star**2 + P_star # Put the pressure back in the flux again, removed expanded aximuthal pressure-gradient term in the azimuthal momentum to reverse the change
    ...
    flux_mom_u -= c * 0.5 * (rho_r * u_r - rho_l * u_l) 
    flux_mom_w -= c * 0.5 * (rho_r * w_r - rho_l * w_l)
