...
grip_const = 0.#100. #50000000. # Arbitrary grip constant in 1/s (I just invented it)
...
def init():
    for i, j in q:
        ...
        if r < 0.2:
            ...
            q[i, j] = [0.3, 0, 0, 0.5]
        if φ > 0.225 and φ < 0.275 and r > 0.5 and r < 0.55:
            q[i, j] = [0.9, 4.9, 0, 1]
        if φ > 0.725 and φ < 0.775 and r > 0.5 and r < 0.55:
            q[i, j] = [0.9, 4.9, 0, 1]
        if φ > 0.425 and φ < 0.575 and r > 0.5 and r < 0.55:
            q[i, j] = [0.9, 0, 4.9, 1]
        if φ > 0.8 and φ < 0.9 and r > 0.5 and r < 0.55:
            q[i, j] = [0.9, -0.5, 5, 1] 
        """
        phi = dφ * j
...
"""
@ti.func
def scalar_laplacian(grad_r: ti.template(), grad_φ: ti.template(), i, j): # Specific to spherical coordinates -- laplacian of a scalar quantity f
    r = get_radius(i)
    divergence_term_r = (1/r**2) * scalar_slope(0, grad_r, i, j)
    divergence_term_φ = (1/r) * scalar_slope(1, grad_φ, i, j)
    scalar_lap_f = divergence_term_r + divergence_term_φ
    return scalar_lap_f

@ti.func
def laplace(): # ------------------------------------ NOTE: highly likely to be incorrect - please fix!
    ...
    for i, j in q:
        r = get_radius(i)
        grad_u_term1[i, j] = r**2 * slope_u_r[i, j] # <--- included r^2 term for the divergence later on
        grad_u_term2[i, j] = (1/r)*slope_u_φ[i, j]

        grad_w_term1[i, j] = r**2 * slope_w_r[i, j] # <--- included r^2 term for the divergence later on
        grad_w_term2[i, j] = (1/r)*slope_w_φ[i, j]
    for i, j in q:
        lap_u = scalar_laplacian(grad_u_term1, grad_u_term2, i, j)
        lap_w = scalar_laplacian(grad_w_term1, grad_w_term2, i, j)
        lap[i, j] = [lap_u, lap_w]

@ti.func
def gradients_and_divergence(): # ------------------------------------ NOTE: highly likely to be incorrect - please fix!
    for i, j in q:
        r = get_radius(i)
        u, w = q[i, j][1], q[i, j][2]
        second_derivative_u_r = scalar_slope(0, slope_u_r, i, j)
        second_derivative_w_φ = scalar_slope(1, slope_w_φ, i, j)
        derivative_r_of_w_φ = scalar_slope(0, slope_w_φ, i, j)
        derivative_φ_of_u_r = scalar_slope(1, slope_u_r, i, j)
        
        grad_div_x = second_derivative_u_r + (2/r)*slope_u_r[i, j] - (2*u)/r - (1/r**2)*slope_w_φ[i, j] + (1/r)*derivative_r_of_w_φ
        grad_div_y = (1/r)*second_derivative_w_φ + (2/r)*slope_u_φ[i, j] + derivative_φ_of_u_r
    ...
    for i, j in pixels:
        ...
        if z.norm() * zoom < 0.5:
            ...
            if show_vorticity:
                ...
                pixels[i, j] = q[r, t][0]/divide_by
        else:
            pixels[i, j] = 0 

gui = ti.GUI('grid projection', (res, res))
z = gui.slider('Display Tolerance', minimum=0.5, maximum=20, step=1)
z.value = init_rho * (1/0.6)
