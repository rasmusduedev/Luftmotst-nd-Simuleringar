...
def laplace():
    for i, j in q: # Initial spatial derivatives
        slope_u_x[i, j] = Δq_x[i, j][1]#scalar_slope(0, u_copy, i, j)
        slope_u_y[i, j] = Δq_y[i, j][1]#scalar_slope(1, u_copy, i, j)
        slope_v_x[i, j] = Δq_x[i, j][2]#scalar_slope(0, v_copy, i, j)
        slope_v_y[i, j] = Δq_y[i, j][2]#scalar_slope(1, v_copy, i, j)
...
def all(result_div: ti.template()):
    for i, j in q:
        ...
        Δq_x[i, j] = vector_minmod(a, b, vec_four)/dx#slope(0, vec_four, q, i, j, 1)

        R, L = boundary_c(j, Ny)
        a, b = q[i, j] - q[i, L], q[i, R] - q[i, j]
        Δq_y[i, j] = vector_minmod(a, b, vec_four)/dy#slope(1, vec_four, q, i, j, 1)
...
def paint():
    for i, j in pixels:
        ...
        if show_vorticity:
            ...
            pixels[i, j] = vorticity 
        else: 
            pixels[i, j] = q[x, y][0] / 2.

gui = ti.GUI('grid projection', (width, height))
